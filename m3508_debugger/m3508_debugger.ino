...
// C620 デバッグ用
int16_t current_rpm_fl = 0.0;
int16_t current_rpm_fr = 0.0;
int16_t current_rpm_rl = 0.0;
int16_t current_rpm_rr = 0.0;
...
void loop() {
  ...
  if (xQueueReceive(rx_queue, &rx_msg, 0) == pdTRUE) {
    ...
    if (rx_msg.id == 0x201) {
      current_rpm_fl = rx_msg.data[2] << 8 | rx_msg.data[3];
    } else if (rx_msg.id == 0x202) {
      current_rpm_fr = rx_msg.data[2] << 8 | rx_msg.data[3];
    } else if (rx_msg.id == 0x203) {
      current_rpm_rl = rx_msg.data[2] << 8 | rx_msg.data[3];
    } else if (rx_msg.id == 0x204) {
      current_rpm_rr = rx_msg.data[2] << 8 | rx_msg.data[3];
    }
  }
